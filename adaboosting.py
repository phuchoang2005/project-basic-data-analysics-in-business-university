# save as predict_expected_returns_adaboost.py
import pandas as pd
import numpy as np
...
def prepare_features(df, n_lags=[1, 2], windows=[5, 10, 20]):
    """
    Tạo đặc trưng (feature) và nhãn (label) cho từng ticker.
    """
...
def train_and_predict_adaboost(df_feats, annualize_factor=252):
    """
    Huấn luyện AdaBoost cho từng ticker và tính tỷ suất lợi nhuận kỳ vọng.
    """
    results = []
    for t, sub in df_feats.groupby('ticker'):
        ...
        # Bỏ qua nếu dữ liệu quá ít hoặc chỉ có 1 nhãn
...
def csv_to_expected_returns(csv_path, out_path='expected_returns_adaboost.csv'):
    ...
    df_res = train_and_predict_adaboost(df_feats)
...
if __name__ == '__main__':
    csv_to_expected_returns("quantumn_clean.csv", "expected-returns/expected_returns_adaBoosting.csv")
