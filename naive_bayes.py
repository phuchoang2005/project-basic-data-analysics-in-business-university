# save as predict_expected_returns_nb.py
import pandas as pd
import numpy as np
...
def prepare_features(df, n_lags=[1, 2], windows=[5, 10, 20]):
    """
    Tạo đặc trưng (features) từ adjclose theo từng ticker.
    """
...
def train_and_predict_nb(df_feats, annualize_factor=252):
    """
    Huấn luyện Gaussian Naive Bayes cho từng ticker và tính tỷ suất lợi nhuận kỳ vọng.
    """
    results = []
    ...
    for t, sub in df_feats.groupby('ticker'):
        ...
        # Lợi nhuận trung bình khi tăng / giảm
...
def csv_to_expected_returns(csv_path, out_path='expected_returns_nb.csv'):
    ...
    df_res = train_and_predict_nb(df_feats)
...
if __name__ == '__main__':
    csv_to_expected_returns("quantumn_clean.csv", "expected-returns/expected_returns_naive_bayes.csv")
